...
import { CLAUDE_BACKEND_ID } from "./adapter";
import { parseClaudeJsonOutput } from "./parser";
import type { ClaudeVersionEvaluation } from "./version";

const execFileAsync = promisify(execFile);

export const CLAUDE_CONNECTION_TEST_PROMPT =
...
export interface ProbeClaudeAuthOptions {
  ...
  versionStatus?: ClaudeVersionEvaluation;
...
async function probeWithStatusCommand(
  ...
  try {
    ...
    if (
      ...
      /unknown command|invalid command|unrecognized|not found/i.test(String(execError.stderr ?? execError.message))
    ) {
      return null;
    }
...
async function probeWithConnectionTest(executable: string, options: ProbeClaudeAuthOptions): Promise<ModelBackendDetectionResult> {
  try {
...
/**
 * Classifies Claude readiness without reading credentials or token files.
 * Prefers `claude auth status --json`; falls back to a minimal `-p` connection-test prompt.
 */
