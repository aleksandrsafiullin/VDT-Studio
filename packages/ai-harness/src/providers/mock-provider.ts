...
function mockAgentDecision(input: unknown): unknown {
  const context = asRecord(input);
  const project = asRecord(context?.currentProject);
  const nodeIds = new Set((Array.isArray(project?.nodes) ? project.nodes : [])
    .flatMap((node) => typeof asRecord(node)?.id === "string" ? [asRecord(node)!.id as string] : []));
  const answers = asRecord(context?.userAnswers) ?? {};
      ...
      questions: [
        {
          id: "payload_per_trip_t",
          question: "What is the average payload per truck trip in tonnes?",
          reason: "Annual hauled tonnes require payload per completed truck trip.",
          required: true,
        ...
        },
        {
          ...
          reason: "Trips per truck require an operating-hours base.",
          required: true,
          expectedAnswerType: "number"
        }
      ]
  ...
  if (!project || nodeIds.size === 0) {
    return {
      type: "call_tool",
      toolName: "vdt.create_draft",
      args: { projectTitle: "Ore haulage Driver Model", rootKpi: "Ore haulage", unit: "tonnes/year", timePeriod: "year", industry: "Mining" },
      statusMessage: "Creating the hauled-tonnes root."
    };
  }
  const add = (nodeId: string, name: string, parentNodeId: string, extra: Record<string, unknown>) => ({
    type: "call_tool",
    toolName: "vdt.add_driver",
    args: { parentNodeId, nodeId, name, ...extra },
  ...
  if (typeof root?.formula !== "string" || !root.formula) {
    return {
      type: "call_tool",
      toolName: "vdt.set_formula",
      args: { nodeId: "ore_haulage", formula: "number_of_trucks * trips_per_truck * payload_per_trip_t" },
      statusMessage: "Setting the hauled-tonnes formula."
    };
  }
  if (!hasTool("vdt.calculate")) {
    return { type: "call_tool", toolName: "vdt.calculate", args: {}, statusMessage: "Calculating the graph." };
  }
  return {
    type: "finish",
    summary: "Built a valid truck haulage VDT.",
    nextSuggestedActions: ["Review payload and operating-hours assumptions."]
  };
}
